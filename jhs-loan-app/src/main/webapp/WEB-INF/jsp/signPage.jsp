...
<div class="signContract">
    <div class="contractDiv">
        ...
        <div class="signContent">
            <p>手机型号：<span><%=data == null ? "" : data.get("device") %></span></p>
            <p>评估价格：<span><%=data == null ? "" : data.get("amount") %></span>元</p>
            <p>租期：<span><%=data == null ? "" : data.get("totalDay") %></span>天</p>
            <p>期数：<span><%=data == null ? "" : data.get("termNum") %></span>期</p>
            <p>押金：<span><%=data == null ? "" : data.get("deposit") %></span>元</p>
            <% if(data != null && data.get("serviceAmount") != null) {%>
                <p>评估费：<span><%=data.get("serviceAmount")%></span>元</p>
            <%}%>
            <p>每期租金：<span><span><%=data == null ? "" : data.get("rent") %></span>元</p>
            <p>银行卡号：<span><span><%=data == null ? "" : data.get("bankNum") %></span></p>
        </div>
        <%--<div class="signInfo">--%>
        ...
        <div class="signInput">
            <input type="checkbox" checked="checked" class="ckeckBox"> <span>我已阅读
                <a class="openNew" href="javascript:;">《手机出售及回购协议》</a>
...
<script type="text/javascript">
    ...
    var perId = '<%=data != null ? data.get("perId") : ""%>';
    var borrId = '<%=data != null ? data.get("borrId") : ""%>';
    var amount = '<%=data != null ? data.get("amount") : ""%>';
    var termNum = '<%=data == null ? "" : data.get("termNum") %>';
    var totalDay = '<%=data == null ? "" : data.get("totalDay") %>';
    var deposit = '<%=data == null ? "" : data.get("deposit") %>';
    var rent = '<%=data == null ? "" : data.get("rent") %>';
    var bankNum = '<%=data == null ? "" : data.get("bankNum") %>';
    var device = '<%=data == null ? "" : data.get("device") %>';
    var token = '<%=data == null ? "" : data.get("token") %>';
    var path = '<%=path%>';
    ...
    var certificateOpen = function (code) {
        if (navigator.userAgent.match(/(iPhone|iPod|iPad);?/i)) {
            ...
            //安卓
            window.yhs.certificateOpen(code);
        }
    };
    ...
    var userAuthOpen = function (code) {
        if (navigator.userAgent.match(/(iPhone|iPod|iPad);?/i)) {
            ...
            //安卓
            window.yhs.userAuthOpen(code);
        }
    };

    var callSign = function (perId,borrId) {
        if (navigator.userAgent.match(/(iPhone|iPod|iPad);?/i)) {
            //苹果
            window.webkit.messageHandlers.callSign.postMessage(perId+","+borrId);
        }else if (navigator.userAgent.match(/android/i)) {
            //安卓
            window.yhs.callSign(perId,borrId);
        }
    };
    var open = function(){
    ...
    window.onload = function(){
        ...
        certificate.onclick = function(){
            if(code == "201"){//通知app
                ...
                certificateOpen("numberCertificate.jsp");
            }
        };

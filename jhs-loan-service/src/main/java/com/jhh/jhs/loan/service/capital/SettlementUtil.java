...
import com.jhh.jhs.loan.service.sms.SmsServiceImpl;
...
public class SettlementUtil {
    ...
    private void sendSMSAndMsgSucc(LoanOrderDO order, BorrowList borrowList, BorrPerInfo bpi) {
        ...
        try {
            LoanOrderDO feeOrder = loanOrderDOMapper.selectSubOrderByPid(order.getId());
            BigDecimal fee = feeOrder.getOptAmount();
            String money = String.format("%.2f", order.getOptAmount().doubleValue() + fee.doubleValue());
            if (order.getTriggerStyle() == PayTriggerStyleEnum.AUTO.getCode() || order.getTriggerStyle() == PayTriggerStyleEnum.BACK_GROUND.getCode()) {
                smsService.sendSms(SmsTemplateEnum.PAY_SUCCESS_REMIND.getCode(), bpi.getPhone(), bpi.getName(), DateUtil.getDateString(order.getUpdateDate(), YSH_DK_SMS_DATE_PATTERN), money);
                userService.setMessage(String.valueOf(order.getPerId()), "5", bpi.getName() + "," + DateUtil.getDateString(order.getUpdateDate()) + "," + money);
            } else if (order.getTriggerStyle() == PayTriggerStyleEnum.USER_TRIGGER.getCode()) {
                smsService.sendSms(SmsTemplateEnum.REPAY_SUCCESS_REMIND.getCode(), bpi.getPhone(), DateUtil.getDateString(order.getUpdateDate(), YSH_DK_SMS_DATE_PATTERN), money);
                userService.setMessage(String.valueOf(order.getPerId()), "10", DateUtil.getDateString(order.getUpdateDate()) + "," + money);
            }
            ...
            if (b.doubleValue() == 0) {
                ...
                userService.setMessage(String.valueOf(order.getPerId()), "13", bpi.getName() + "," + DateUtil.getDateString(order.getUpdateDate()) + "," + borrowList.getBorrNum());
            }
        } catch (Exception e) {
    ...
    private void sendSMSAndMsgFail(LoanOrderDO order, BorrowList borrowList, BorrPerInfo bpi) {

        try {
            LoanOrderDO feeOrder = loanOrderDOMapper.selectSubOrderByPid(order.getId());
            BigDecimal fee = feeOrder.getOptAmount();
            String money = String.format("%.2f", order.getOptAmount().doubleValue() + fee.doubleValue());
            if (order.getTriggerStyle() == PayTriggerStyleEnum.USER_TRIGGER.getCode()) {
                smsService.sendSms(SmsTemplateEnum.PAY_FAIL_REMIND.getCode(), bpi.getPhone(), bpi.getName(), DateUtil.getDateString(order.getUpdateDate(), YSH_DK_SMS_DATE_PATTERN), money);
                userService.setMessage(String.valueOf(order.getPerId()), "9", bpi.getName() + "," + DateUtil.getDateString(order.getUpdateDate()) + "," + money);
            }
        } catch (Exception e) {
            logger.error("短信发送失败:orderId" + order.getId(), e);
        }

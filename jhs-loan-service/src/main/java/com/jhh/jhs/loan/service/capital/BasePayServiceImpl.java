...
import com.jhh.jhs.loan.common.enums.TradeStatusEnum;
...
public class BasePayServiceImpl extends BaseServiceImpl {
        ...
        if (!verifyZFBChannel(request.getPayChannel())) {
            if (StringUtils.isEmpty(request.getBankNum())) {
                ...
                if (bank == null) {
                    ...
                    NoteResult noteResult = bankService.payCenterBindCard(vo);
    ...
    protected LoanOrderDO saveDeductLoanOrder(AgentDeductRequest request, Integer perId, Integer borrowId, String type, int payChannel) {
        ...
        loanOrder.setSerialNo(SerialNumUtil.createByType("JHS" + String.format("%02d", Integer.parseInt(type))));
    ...
    protected NoteResult canPayCollect(BorrowList borrowList, double thisAmount, String type) {
        ...
        if ("1".equals(type)) {
            ...
            //判断用户最大应还金额
            String totalAmount = borrowListMapper.getTotalLeft(String.valueOf(borrowList.getId()));
            if (thisAmount > Double.parseDouble(totalAmount)) {
                result.setCode("201");
            ...
            result = ysbpayService.canPayCollect(borrowList.getId().toString(), thisAmount);
        } else {
            result.setCode("201");
    ...
    protected LoanOrderDO savePayLoanOrder(PersonInfoDto dto, String type, int triggerStyle, int payChannel) {
        ...
        loanOrderDO1.setSerialNo(SerialNumUtil.createByType("JHS" + String.format("%02d", Integer.parseInt(type))));

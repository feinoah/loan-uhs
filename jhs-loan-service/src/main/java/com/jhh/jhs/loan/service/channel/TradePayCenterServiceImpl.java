...
import com.jhh.jhs.loan.api.entity.capital.LoanOrderVO;
...
public class TradePayCenterServiceImpl extends BasePayServiceImpl implements TradePayService, TradeBatchStateService {
    ...
    public ResponseDo<String> postDeduct(TradeVo vo) {
        ...
        vo = userService.chooseBankRouting(vo);
        Bank bank = bankMapper.selectByBankNumAndStatus(vo.getBankNum());
        if (PayCenterChannelConstant.PAY_CHANNEL_HLB_QUICK.equals(vo.getPayChannel())) {
            if (bank.getQuickPayStatus() == null) {
                // 如果没有绑定合利宝快捷支付,则进行绑卡
                ...
                bindCardVo.setPer_id(String.valueOf(vo.getPerId()));
                bindCardVo.setTokenKey("");
                bindCardVo.setToken("");
                bindCardVo.setValidateCode(vo.getValidateCode());
                bindCardVo.setExtension("{\"userId\":\"" + bank.getPerId() + "\"}");
                bindCardVo.setChannel(PayCenterChannelConstant.PAY_CHANNEL_HLB_QUICK);
                NoteResult noteResult = bankService.payCenterBindCard(bindCardVo);
                if (!CodeReturn.SUCCESS_CODE.equals(noteResult.getCode()) && !CodeReturn.REPEAT_BIND.equals(noteResult.getCode())) {
                    log.error("绑定合利宝快捷支付失败:" + noteResult.getInfo());
                    return ResponseDo.newFailedDo(noteResult.getInfo());
                }
                // 更新银行表快捷支付状态
                bank.setQuickPayStatus(QuickPayBindStatusConstant.QUICK_PAY_HLB);
        ...
        if (PayTriggerStyleEnum.USER_TRIGGER.getCode().equals(vo.getTriggerStyle())) {
            payRequest.setAppId(Constants.PayStyleConstants.YHS_REPAY_SWITCH);
        } else {
            payRequest.setAppId(Constants.PayStyleConstants.YHS_DEDUCT_APPID);
        }
    ...
    public ResponseDo<?> batchDeduct(TradeBatchVo vo) {
        ...
        request.setAppId(Constants.PayStyleConstants.YHS_BATCH_APPID);
    ...
    public ResponseDo<PayResponse> batchState(List<String> serialNo) {
        ...
        batchQueryRequest.setAppId(Constants.PayStyleConstants.YHS_BATCH_APPID);
    ...
    private List<Order> saveBatchOrder(TradeBatchVo vo) {
        ...
        vo.getDeduct().forEach(v -> {
            ...
            order.setAppId(Constants.PayStyleConstants.YHS_BATCH_APPID);
            //传入支付渠道
            Map<String, Object> channelMap = new HashMap<>();
            if (StringUtils.isNotEmpty(vo.getPayChannel())) {
                channelMap.put(vo.getPayChannel(), new JSONObject());
            ...
            BankList list = bankListMapper.selectByPrimaryKey(Integer.parseInt(bank.getBankId()));
            bankInfo.setBankNo(list.getBankCode());

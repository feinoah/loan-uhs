...
import com.jhh.jhs.loan.manage.service.loan.LoanCompanyService;
...
public class AuthController {
    ...
    public @ResponseBody String loadCollectorsLevels(HttpServletRequest request){
        ...
        try{
            Map<String,Object> queryMap = QueryParamUtils.getargs(request.getParameterMap());
            PageInfo<Collectors> infos = collectorsService.selectCollectors(queryMap,offset,size,userNo);
            return JSON.toJSONString(infos);
        }catch (Exception e){
            e.printStackTrace();
            return JSON.toJSONString(new PageInfo<Collectors>());
        }
    }
    ...
    public @ResponseBody Result editCollectorsLevel(Collectors collectors){
        Result result = new Result();
        try{

...
import com.jhh.jhs.loan.manage.service.common.RedisService;
...
public class LoanManagementServiceImpl implements LoanManagementService {
    ...
    public Result askCollection(AskCollection askCollection) {
        ...
        if (StringUtils.isEmpty(askCollection.getPayChannel())) {
            // 查看扣款渠道是否是合利宝快捷支付
            CodeValue codeValue = codeValueMapper.selectByCodeType("pay_style_deduct");
            defaultChannel = "0".equals(codeValue.getCodeCode()) && PayCenterChannelConstant.PAY_CHANNEL_HLB_QUICK.equals(codeValue.getMeaning());
        }

        // 判断是否是合利宝快捷支付如果是判断是否绑定银行卡
        //TODO:临时解决方案 根据用户银行卡选择渠道路由
        TradeVo tradeVo = new TradeVo();
        tradeVo.setBankNum(askCollection.getBankNum());
        tradeVo = userService.chooseBankRouting(tradeVo);
        askCollection.setPayChannel(tradeVo.getPayChannel()==null?askCollection.getPayChannel():tradeVo.getPayChannel());
        if (PayCenterChannelConstant.PAY_CHANNEL_HLB_QUICK.equals(askCollection.getPayChannel()) || defaultChannel) {
            Bank bank = bankInfoMapper.selectByBankNumAndStatus(askCollection.getBankNum());
            // TODO 添加拉卡拉快捷支付还需额外判断
            if (bank ==null || bank.getQuickPayStatus() == null) {
                result.setCode(Result.FAIL);
                result.setMessage("未绑定合利宝快捷支付");
                return result;
            }
        }
        //查询催收人
        ...
        if (CodeReturn.success == noteResult.getCode()) {
            ...
            if (StringUtils.isNotEmpty(askCollection.getCreateUser())) {
                ...
                Collectors level = collectorsMapper.selectOne(queryLevel);
                if (level != null) {
                    //查询受理的订单
    ...
    @Override
    public PageInfo<Collectors> selectReceiptUsers(Map<String, Object> queryParams, String userNo, Integer type, int offset, int size) {

        Example example = new Example(Collectors.class);
        example.createCriteria().andEqualTo("userSysno", userNo);
        List<Collectors> collectorList = collectorsMapper.selectByExample(example);
        if (collectorList == null || collectorList.isEmpty()) {
            return null;
        }

        Collectors collectors = collectorList.get(0);
        ...
        PageHelper.offsetPage(offset, size);
        return new PageInfo<Collectors>(collectorsMapper.selectDsUsers(queryParams));
    }

    @Override
    public PageInfo<Collectors> selectReceiptUsers(String type, int offset, int size) {
        Map<String, Object> quaryParams = new HashMap<>();
        quaryParams.put("levelType", "1");
        quaryParams.put("type", type);
        PageHelper.offsetPage(offset, size);
        return new PageInfo<Collectors>(collectorsMapper.selectDsUsers(quaryParams));
    }
...
//            }
            ...
            if (borrowList.getOverdueDays() > 7) {
                reduceAmount = borrowList.getActRepayAmount() - borrowList.getActReduceAmount() >= borrowList.getActRepayAmount() ? borrowList.getSurplusAmount() : borrowList.getSurplusAmount() + borrowList.getActRepayAmount() - borrowList.getPayAmount() - borrowList.getActReduceAmount();
            } else {
                reduceAmount = borrowList.getPenalty() - borrowList.getActReduceAmount();
                if (reduceAmount > borrowList.getSurplusAmount()) {
                    reduceAmount = borrowList.getSurplusAmount();
                }
            }
